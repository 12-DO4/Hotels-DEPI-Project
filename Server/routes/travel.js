...
router.post("/", upload.single("coverImg"), async (req, res) => {
  ...
  if (error) {
    ...
    return res
      .status(400)
      .render("add_travel", { errors: errorMsg, travel: req.body });
  }
...
router.put("/:id", upload.single("coverImg"), async (req, res) => {
  ...
  if (error) {
    ...
    return res
      .status(400)
      .render("edit_travel", { errors: errorMsg, travel: req.body });
  }
  ...
  if (req.file) updatedData.coverImg = req.file.path;

  travel = await Travel.findByIdAndUpdate(req.params.id, updatedData, {
    new: true,
  });
